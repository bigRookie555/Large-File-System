...
namespace skx{
	namespace LFS{
		...
		int IndexHandle::create(const uint32_t logic_block_id,const int32_t bucket_size,const MMapFileOperation map_option){
			int ret=TFS_SUCCESS;
			...
			if(file_size<0) return TFS_ERROR;
				...
				i_Header.block_info_.block_id_=login_block_id;
				...
				memccpy(ini_data,&i_header,sizeof(IndexHeader));
		...
		int IndexHandle::load(const uint32_t logic_block_id,const int32_t bucket_size,const MMapFileOperation map_option){
			...
			MMapFileOperation tmp_map_option = map_option;
