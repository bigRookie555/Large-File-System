...
namespace skx{
	namespace LFS{
		int MMapFileOperation::mmap_file(const MMapOption &mmap_option){
			...
			int fd=check_file();
			if(fd<0){
				fprintf(stderr,"MMapFileOperation::mmap_file check_file error:%s",strerror(errno));
				return TFS_ERROR;
			}
		...
		int MMapFileOperation::pread_file(char *buf,const int32_t size,const int64_t offset){
			...
			if(is_mapped_&&(offset+size)>map_file_->get_size()){
				if(debug) fprintf(stderr,"MMapFileOperation::pread,size:%d,offset:%"__PRI64_PREFIX"d,map file size:%d.need remap",size,offset,map_file_->get_size());
		...
		int MMapFileOperation::pwrite_file(const char *buf,const int32_t size,const int64_t offset){
			//1.已经映射了
			if(is_mapped_&&(offset+size)>map_file_->get_size()){
				if(debug) fprintf(stderr,"MMapFileOperation::pwrite,size:%d,offset:%"__PRI64_PREFIX"d,map file size:%d.need remap",size,offset,map_file_->get_size());
				map_file_->remap_file();
			} //尝试扩大内存一次
